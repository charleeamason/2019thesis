...
int main(int argc, char** argv) try {
  ...
  int nmu(50); //set smaller nmu when testing
  ...
  char spectra_file[256] = "spectrum.txt"; 
  ...
  const int E_len = 10; //length of energy array
  double E_n = 1.0; //starting energy value (keV)
  const double E_bin = 0.5; //bin width
  double E_spec[E_len] = {0}; //initialize array of energies for spectrum file (keV)
  ...
  std::cout << "inclination = " << incl << " radians" << std::endl;
  ...
  double dmu,dphi, P2;
  dmu = 1.0/(nmu*1.0); //set smaller nmu when testing
  ...
  // Loop through the star's latitudes
   for (int j(0); j<nmu; j++){
	 ...
	 if (curve.dOmega_s[i] != 0.0){
	   ...
	   dFlux_mu += curve.dOmega_s[i] * pow(eta,3) * BlackBody(Temperature,E_n*redshift/curve.eta[i]);
...
//spectra.open(spectra_file, std::ios::app);

  //  out = fopen(out_file, "a");
	
 // Print out information about the model
//spectra<<"Energy =" <<E_spec<<"keV"<<std::endl;
